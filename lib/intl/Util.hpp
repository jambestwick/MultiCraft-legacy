...
namespace libintllite {
	...
	namespace internal {
		...
		static inline bool
		readUIn32FromFile(FILE *fileHandle, bool needsBeToLeConversion, uint32_t &outReadUInt32) {
		...
		static bool loadMoFileStringsToArray(FILE *moFile,
...

			{
				...
				uint32_t stringCharsArraySize =
						lastStringOffset + stringsLengthsArray[numberOfStrings - 1] + 1 -
						firstStringOffset;
				...
				if (!stringCharsArray)
					return false;
				if (fread(stringCharsArray, 1, stringCharsArraySize, moFile) !=
					stringCharsArraySize)
					return false;
				...
				for (uint32_t i = 0; i < numberOfStrings; i++) {
					...
					outStringsFromMoFileArray[i] = std::string(stringsCharsArrayIter,
															   currentStrEndIter);
					/* skip the NULL char at the end of the string */
					stringsCharsArrayIter = currentStrEndIter + 1;
				}
			}
